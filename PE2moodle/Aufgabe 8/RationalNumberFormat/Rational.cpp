...
std::string Rational::toString(std::string LANG) {
...
void Rational::setFormat(std::string LANG) {
    if (LANG == "EN")
        nfEN = new NumberFormatEN();
    else if (LANG == "DE")
        nfDE = new NumberFormatDE();

}

Rational Rational::parse(std::string val) {
    nfEN = new NumberFormatEN();
    this->_counter = nfEN->parse(val)._counter;
    this->_denomi = nfEN->parse(val)._denomi;
    return *this;
